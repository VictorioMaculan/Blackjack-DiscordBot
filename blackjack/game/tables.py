...
class Table():
    ...
    async def start_game(self, client: discord.Client):
        ...
        async for round in ut.Alist(range(1, 3)):
            ...
            if all_players_lost or self.allPlayersBlackjacked():
                async for player in self.players:
                    player.result += sum([hand.bet for hand in player.hands]
                                         ) * (-1 if all_players_lost else 1)
                break
            ...
            if round == 1:
                ...
                dealer.hand.append(
                    Card(image='blackjack/cards/back.png', value=None))  # Hidden Card
                ...
                while True:
                    ...
                    # Dealer got a Blackjack (Game Ends)
                    if dealer.total == 21:
                    ...
                    async for player in self.players:
                        async for hand in player.hands:
                            if hand.total == dealer.total:
                                break
                            player.result += hand.bet if (
                                hand.total > dealer.total and hand.total <= 21) else hand.bet*-1
                    break
            ...
            async for player in self.players:
                player: Player
                # The main hand gets two cards
                player.hands[0].draw_cards(self.deck, amount=2)
                ...
                while abs(hand_num) != len(player.hands):
                    ...
                    try:
                        while True:
                            ...
                            # Ending the turn because the player doubled
                            if output[0].emoji == '💸':
                                break
        ...
        msg = discord.Embed(title='Match Summary',
                            colour=discord.Colour.gold(), description='')
        async for player in self.players:
