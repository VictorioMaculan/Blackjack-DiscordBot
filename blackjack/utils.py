...
def generateCards(cards: list, gap=8, Cwidth=40, Cheight=58):
    ...
    img = Image.new(mode='RGBA', size=(
        len(cards)*(Cwidth+gap)+gap, Cheight+gap*2), color='#216b19')
    pencil = ImageDraw.Draw(img)
    pencil.rectangle(xy=(0, 0, img.width-1, img.height-1),
                     outline='#00260a', width=3)

    # Putting the cards
    for i, card in enumerate(cards):
        img.paste(Image.open(card.image), box=(gap+(Cwidth+gap) *
                  i, gap, Cwidth*(i+1)+(gap*(i+1)), gap+Cheight))
    return img
